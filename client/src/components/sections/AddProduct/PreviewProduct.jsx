...
import { Counter, IconButton, IconButton2, ImageSlider, StarRating } from '../../ui';
import { Facebook, Heart, Instagram, ShoppingCart, Twitter } from 'lucide-react';

const PreviewProduct = ({formData}) => {
    const [count, setCount] = useState(0);
    const images = [formData.previewImage, ...formData.additionalImages ]
  ...
  return (
     ...
     <div className="w-full lg:w-1/2">
       <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-3">
         <h1 className="text-2xl sm:text-3xl font-semibold">{formData.title}</h1>
         <div className={`text-sm rounded-sm px-2 py-1 w-fit ${formData.quantity > 0 ? "bg-primary/10 text-primary" : "bg-danger/10 text-danger"}`}>
       ...
       <div className="flex flex-col sm:flex-row w-full sm:items-center gap-4 sm:gap-7 mb-5">
         <div className="flex items-center gap-1.5">
           ...
           <p className="text-sm text-gray-500">{formData.ratingCount} Reviews</p>
         </div>
       ...
       <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-5">
         <div className="gap-1 flex items-center">
           <span className={`${formData.discount.isActive ? "line-through text-lg sm:text-xl text-gray-300" : "text-xl sm:text-2xl font-medium text-hard-primary"} `}>${Number.parseInt(formData.price).toFixed(2)}</span>
         ...
         <div className="text-sm rounded-full text-danger px-3 py-1 bg-danger/10 font-medium w-fit">
           {formData.discount.isActive && `${formData.discount.percentage}% Off`}
         </div>
       </div>
       ...
       <div className="w-full my-6">
         <div className="flex items-center justify-between gap-4">
           <div className="text-sm">
             Brand:{" "}
             <span className="text-hard-primary font-medium">{formData.brand}</span>
           </div>
